...
void matchBuyOrder(connection* C,long tran_id){
    work W(*C);
    stringstream query;
    query << "SELECT SYM_NAME, ACT_ID, TRAN_LIMIT_PRICE, TRAN_AMOUNT, TRAN_TIME FROM TRANSACTIONS "
    ...
    while(tran_amount>0){ 
        ...
        matching << "SELECT TRAN_ID, ACT_ID, TRAN_LIMIT_PRICE, TRAN_AMOUNT, TRAN_TIME FROM TRANSACTIONS "
        ...
        for(auto it = matching_res.begin(); it != matching_res.end()&&tran_amount>0; ++it){
            ...
            if((-seller_amount)<tran_amount){
                ...
                createOrder(W, seller_tran_id, seller_amount, seller_price, cur_time);
                updateTransaction(W,tran_id,tran_amount);
                seller_amount=0;
                updateTransaction(W,seller_tran_id,seller_amount);
                
            }
            else{
                // cut sell order
                seller_amount+=tran_amount;
                ...
                createOrder(W, seller_tran_id, -tran_amount, seller_price, cur_time);
...
void matchSellOrder(connection* C, long tran_id){
    ...
    while(tran_amount<0){ 
        ...
        << "WHERE SYM_NAME = " << W.quote(sym) << " AND TRAN_STATUS = " << W.quote("OPEN") 
        << " AND TRAN_AMOUNT > 0 AND TRAN_LIMIT_PRICE >= " << W.quote(limit_price)
        << "ORDER BY TRAN_LIMIT_PRICE DESC, TRAN_TIME ASC;";
        result matching_res(W.exec(matching.str()));
        for(auto it = matching_res.begin(); it != matching_res.end()&&tran_amount<0; ++it){
            ...
            if((buyer_amount)<(-tran_amount)){
                ...
                createOrder(W, buyer_tran_id, buyer_amount, buyer_price, cur_time);
                updateTransaction(W,tran_id,tran_amount);
                buyer_amount = 0;
                updateTransaction(W,buyer_tran_id,buyer_amount);
                
            }
            else{
                // cut buy order
                buyer_amount += tran_amount;
                ...
                createOrder(W, buyer_tran_id, -tran_amount, buyer_price, cur_time);
                tran_amount = 0;
                updateTransaction(W,tran_id,tran_amount);
                updateTransaction(W,buyer_tran_id,buyer_amount);
